...
#Το module os ανήκει στην βιβλιοθήκη της Python και περιέχει εντολές που αλληλεπιδρούν με το λειτουργικό σύστημα όπως
#για παράδειγμα η πλοήγηση σε φακέλους ή η πρόσβαση σε αρχεία.
import os

#Το module "sys" ανήκει στην βιβλιοθήκη της Python και περιέχει εντολές που επιτρέπουν την πρόσβαση σε ορίσματα της γραμμής
...
#Εισαγωγή της συνάρτησης (function) "load_img" απο το "tensorflow.keras.preprocessing.image" module. Η συνάρτηση αυτή
...
#Εισαγωγή της κλάσης(class) "LabelBinarizer" απο το "sklearn.preprocessing" module. Η κλάση αυτή χρησιμοποιείται για τη
...
#Εισαγωγή της συνάρτησης "to_categorical" απο το "tensorflow.keras.utils" module. Η συνάρτηση αυτή μετατρέπει ακέραιες
...
#Εισαγωγή της συνάρτησης "train_test_split" απο το "sklearn.model_selection" module. Η συνάρτηση αυτή χωρίζει το σύνολο
#δεδομένων (image dataset) σε υποσύνολα εκπαίδευσης (training set/subset) και δοκιμής (testing set/subset).
from sklearn.model_selection import train_test_split

#Εισαγωγή της κλάσης "ImageDataGenerator" απο το "tensorflow.keras.preprocessing.image" module. Η κλάση αυτή αυξάνει
...
#Εισαγωγή της αρχιτεκτονικής του μοντέλου "MobileNetV2" από το "tensorflow.keras.applications" module. Το MobileNetV2
...
#Εισαγωγή του επιπέδου (layer) "Input" απο το "tensorflow.keras.layers" module. Το επίπεδο αυτό αντιπροσωπεύει την είσοδο
#στο πλήρως συνδεδεμένο νευρωνικό δίκτυο (Fully Connected Neural Network) του μοντέλου, ορίζοντας το σχήμα και τον τύπο
#των δεδομένων εισόδου.
from tensorflow.keras.layers import Input

#Εισαγωγή του επιπέδου "AveragePooling2D" απο το "tensorflow.keras.layers" module. Το επίπεδο αυτό εκτελεί μέση συγκέντρωση
...
#Εισαγωγή του επιπέδου "Flatten" απο το "tensorflow.keras.layers" module. Το επίπεδο αυτό μετατρέπει την πολυδιάστατη είσοδο
...
#Εισαγωγή του επιπέδου "Dense" απο το "tensorflow.keras.layers" module. Το επίπεδο αυτό αντιπροσωπεύει ένα πλήρως συνδεδεμένο
#επίπεδο, όπου κάθε νευρώνας συνδέεται με κάθε νευρώνα του προηγούμενου επιπέδου.
from tensorflow.keras.layers import Dense

#Εισαγωγή του επιπέδου "Dropout" απο το "tensorflow.keras.layers" module. Το επίπεδο αυτό ρυθμίζει τυχαία ένα μέρος των
#εισόδων του επόμενου επιπέδου στο 0 κατά τη διάρκεια της εκπαίδευσης, κάτι που βοηθά στην αποφυγή της υπερβολικής
#προσαρμογής (overfitting).
from tensorflow.keras.layers import Dropout

#Εισαγωγή της κλάσης "Model" απο το "tensorflow.keras.models" module. Η κλάση αυτή δημιουργεί ένα αντικείμενο μοντέλου που
#καθορίζει την αρχιτεκτονική ενός νευρωνικού δικτύου.
from tensorflow.keras.models import Model

#Εισαγωγή του βελτιστοποιητή (optimizer) "Adam" από το "tensorflow.keras.optimizers" module. Ο Adam είναι ένας αλγόριθμος
#βελτιστοποίησης που χρησιμοποιείται συνήθως για την εκπαίδευση μοντέλων βαθιάς μάθησης (deep learning).
from tensorflow.keras.optimizers import Adam

#Εισαγωγή των συναρτήσεων "roc_curve" και "auc" απο το "sklearn.metrics" module. Οι συναρτήσεις αυτές  υπολογίζουν την
#καμπύλη ROC και την περιοχή AUC (AUC area) κάτω από την καμπύλη αυτή για την αξιολόγηση μοντέλων ταξινόμησης.
from sklearn.metrics import roc_curve, auc

#Εισαγωγή της συνάρτησης "classification_report" απο το "sklearn.metrics" module. Η συνάρτηση αυτή Δημιουργεί μια αναφορά
#με διάφορες μετρήσεις ταξινόμησης, όπως ακρίβεια, ανάκληση και βαθμολογία F1, για την αξιολόγηση της απόδοσης ενός μοντέλου
#ταξινόμησης.
from sklearn.metrics import classification_report

#Εισαγωγή του module "pyplot" απο τη βιβλιοθήκη "matplotlib" με το ψευδώνυμο "plt". Το Matplotlib είναι μια βιβλιοθήκη
...
for dataset_class in dataset_classes:
	...
	for img_name in os.listdir(path):
		...
		#οι εικόνες έτσι ώστε να έχουν την ίδια διάσταση αλλά με το ίδιο aspect ratio που είχαν.
		image = load_img(img_path, target_size=(IMAGE_SIZE, IMAGE_SIZE))

		#Μετατροπή της εικόνας που βρίσκεται στο image σε μορφή πίνακα (array) για να μπορεί
		#να επεξεργαστεί αργότερα πιο εύκολα. Ένα κομμάτι του array αυτού θα έχει για
