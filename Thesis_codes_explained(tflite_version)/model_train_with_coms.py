#Προσθήκη όλων των απαραίτητων πακέτων δηλαδή των βιβλιοθηκών (libraries), ενοτήτων (modules), πλαισίων (frameworks)  ή μερος αυτών
...
#Σε άλλες περιπτώσεις εισάγονται μέρη αυτών που θα χρειαστούν ή κάποιες συναρτήσεις (functions) τους ή μεθόδους (methods) τους.
#Για την εισαγωγή ενός μόνο μέρους του πακέτου χρησιμοποιείται η σύνταξη "from όνομα_επιθυμητού_πακέτου import όνομα_επιθυμητού_μέρους_του". Επίσης
...
#Η ενότητα os ανήκει στην βιβλιοθήκη της Python και περιέχει εντολές που αλληλεπιδρούν με το λειτουργικό σύστημα όπως
#για παράδειγμα η πλοήγηση σε φακέλους ή η πρόσβαση σε αρχεία.
import os

#Η ενότητα "sys" ανήκει στην βιβλιοθήκη της Python και περιέχει εντολές που επιτρέπουν την πρόσβαση σε ορίσματα της γραμμής
#εντολών (command line), ειδικές παραμέτρους του συστήματος και συναρτήσεις για τον χειρισμό του περιβάλλοντος της Python.
import sys

#Το "tensorflow" είναι ένα πλάισιο ανοικτού κώδικα (open-source code) που περιέχει εντολές με σκοπό τη
#δημιουργία και εκπαίδευση μοντέλων μηχανικής μάθησης (machine learning models).
import tensorflow

#Εισαγωγή της συνάρτησης "load_img" απο την "tensorflow.keras.preprocessing.image" ενότητα. Η συνάρτηση αυτή
#φορτώνει ένα αρχείο εικόνας απο τον τοπικό δίσκο του υπολογιστή μέσα στον κώδικα.
from tensorflow.keras.preprocessing.image import load_img

#Εισαγωγή της συνάρτησης "img_to_array" απο την "tensorflow.keras.preprocessing.image" ενότητα. Η συνάρτηση αυτή, μετατρέπει
...
#Εισαγωγή της συνάρτησης "preprocess_input" απο την "tensorflow.keras.applications.mobilenet_v2" ενότητα. Η συνάρτηση αυτή
...
#Εισαγωγή της κλάσης (class) "LabelBinarizer" απο την "sklearn.preprocessing" ενότητα. Η κλάση αυτή χρησιμοποιείται για τη
#δυαδοποίηση (binarizing) των ετικετών (labels) ή τη μετατροπή κατηγορικών ετικετών (categorical labels) σε δυαδικά διανύσματα.
from sklearn.preprocessing import LabelBinarizer

#Εισαγωγή της συνάρτησης "to_categorical" απο την "tensorflow.keras.utils" ενότητα. Η συνάρτηση αυτή μετατρέπει ακέραιες
#ετικέτες σε κωδικοποιημένα διανύσματα, της μεθόδου one-hot encoding.
...
#Εισαγωγή της συνάρτησης "train_test_split" απο την "sklearn.model_selection" ενότητα. Η συνάρτηση αυτή χωρίζει το σύνολο
#δεδομένων (image dataset) σε υποσύνολα εκπαίδευσης (training set/subset) και δοκιμής (testing set/subset).
from sklearn.model_selection import train_test_split

#Εισαγωγή της κλάσης "ImageDataGenerator" απο την "tensorflow.keras.preprocessing.image" ενότητα. Η κλάση αυτή αυξάνει
#τα δεδομένα, δημιουργώντας ομάδες αυξημένων εικόνων για την εκπαίδευση μοντέλων βαθιάς μάθησης (deep learning models).
from tensorflow.keras.preprocessing.image import ImageDataGenerator

#Εισαγωγή της αρχιτεκτονικής του μοντέλου "MobileNetV2" από την "tensorflow.keras.applications" ενότητα. Το MobileNetV2
#είναι μια προεκπαιδευμένη αρχιτεκτονική συνελικτικού νευρωνικού δικτύου που μπορεί να χρησιμοποιηθεί σε περιπτώσεις
#ταξινόμησης εικόνων.
from tensorflow.keras.applications import MobileNetV2

#Εισαγωγή του επιπέδου (layer) "Input" απο την "tensorflow.keras.layers" ενότητα. Το επίπεδο αυτό αντιπροσωπεύει την είσοδο
#στα κρυφά επίπεδα του νευρωνικού δικτύου του μοντέλου, ορίζοντας το σχήμα και τον τύπο των δεδομένων εισόδου.
from tensorflow.keras.layers import Input

#Εισαγωγή του επιπέδου "AveragePooling2D" απο την "tensorflow.keras.layers" ενότητα. Το επίπεδο αυτό εκτελεί μέση συγκέντρωση
#(Average Pooling), η οποία μειώνει τις χωρικές διαστάσεις των δεδομένων εισόδου λαμβάνοντας το μέσο όρο κάθε παραθύρου
#συγκέντρωσης.
from tensorflow.keras.layers import AveragePooling2D

#Εισαγωγή του επιπέδου "Flatten" απο την "tensorflow.keras.layers" ενότητα. Το επίπεδο αυτό μετατρέπει την πολυδιάστατη είσοδο
#σε ένα μονοδιάστατο διάνυσμα, σε κατάλληλη δηλαδή μορφή για την εισαγωγή του στο επόμενο επίπεδο που είναι πλήρως συνδεδεμένο
#με το γειτονικό του.
from tensorflow.keras.layers import Flatten

#Εισαγωγή του επιπέδου "Dense" απο την "tensorflow.keras.layers" ενότητα. Το επίπεδο αυτό αντιπροσωπεύει ένα πλήρως συνδεδεμένο
#επίπεδο, όπου κάθε νευρώνας συνδέεται με κάθε νευρώνα του προηγούμενου επιπέδου.
from tensorflow.keras.layers import Dense

#Εισαγωγή του επιπέδου "Dropout" απο την "tensorflow.keras.layers" ενότητα. Το επίπεδο αυτό ρυθμίζει τυχαία ένα μέρος των
#εισόδων του προηγούμενου επιπέδου στο 0 κατά τη διάρκεια της εκπαίδευσης, κάτι που βοηθά στην αποφυγή της υπερπροσαρμογής (overfitting).
from tensorflow.keras.layers import Dropout

#Εισαγωγή της κλάσης "Model" απο την "tensorflow.keras.models" ενότητα. Η κλάση αυτή δημιουργεί ένα αντικείμενο μοντέλου που
#καθορίζει την αρχιτεκτονική ενός νευρωνικού δικτύου.
from tensorflow.keras.models import Model

#Εισαγωγή του βελτιστοποιητή (optimizer) "Adam" από την "tensorflow.keras.optimizers" ενότητα. Ο Adam είναι ένας αλγόριθμος
#βελτιστοποίησης που χρησιμοποιείται συνήθως για την εκπαίδευση μοντέλων βαθιάς μάθησης (deep learning).
from tensorflow.keras.optimizers import Adam

#Εισαγωγή των συναρτήσεων "roc_curve" και "auc" απο την "sklearn.metrics" ενότηα. Οι συναρτήσεις αυτές  υπολογίζουν την
#καμπύλη ROC και την περιοχή AUC (AUC area) κάτω από την καμπύλη αυτή για την αξιολόγηση μοντέλων ταξινόμησης.
from sklearn.metrics import roc_curve, auc

#Εισαγωγή της συνάρτησης "classification_report" απο την "sklearn.metrics" ενότητα. Η συνάρτηση αυτή δημιουργεί μια αναφορά
#με διάφορες μετρικές αξιολόγησης, όπως η  ακρίβεια,η ανάκληση και η βαθμολογία F1, για την αξιολόγηση της απόδοσης ενός μοντέλου
#ταξινόμησης.
from sklearn.metrics import classification_report

#Εισαγωγή της ενότητας "pyplot" απο τη βιβλιοθήκη "matplotlib" με το ψευδώνυμο "plt". Το Matplotlib είναι μια βιβλιοθήκη
#σχεδίασης για την γλώσσα προγραμματισμού Python και το pyplot παρέχει μια απλή διεπαφή για τη δημιουργία διαφόρων τύπων
#γραφημάτων και απεικονίσεων.
import matplotlib.pyplot as plt

#Αρχικοποίηση των υπερπαραμέτρων ρυθμός εκπαίδευσης (learning rate), εποχές (epochs), μέγεθος υποσυνόλου δεδομένων (batch size)
#και της παραμέτρου νέων διαστάσεων εικόνας (image size). Κάνοντας αλλαγές στις παραμέτρους αυτές,
...
#1)Ρυθμός εκπαίδευσης (INIT_LR): Είναι μια υπερπαράμετρος που καθορίζει πόσο γρήγορα ή αργά η συνάρτηση βελτιστοποίησης
#του σφάλματος που έχουμε επιλέξει (Adam), κατεβαίνει την καμπύλη σφάλματος. Συνήθως η τιμή της βρίσκεται ανάμεσα στο
#0.0001 and 0.01.
#2)Εποχές (EPOCHS): Είναι μια υπερπαράμετρος η οποία ορίζει τον αριθμό των επαναλήψεων που θα χρειαστεί να εκτελεστούν
#για την εκπαίδευση του μοντέλου.
#3)Μέγεθος υποσυνόλου δεδομένων (BS): Είναι μια υπερπαράμετρος η οποία ομαδοποιεί τα δεδομένα εκπαίδευσης (train_images, train_labels) που
...
#Επίσης το αλφαριθμητικό (string) θα περιέχει το όνομα του φακέλου "models/", που θα περιέχει τον νέο υποφάκελο, σε συνδυασμό με το
...
if os.path.exists(folder_of_model):
	...
	#χαρακτήρες του αλφαριθμητικού "folder_of_model" ώστε να μην εμφανιστεί στην οθόνη το "models/", αλλιώς θα εμφανιζόταν η τοποθεσία
...
#Δημιουργία μιας μεταβλητής (αλφαριθμητικού περιεχομένου) με περιεχόμενο την τοποθεσία των εικόνων,
...
#πιο συγκεκριμένα ως διανύσματα.
data = []

#Δημιουργία μιας λίστας, που αργότερα θα περιέχει για κάθε μία απο τις εικόνες της λίστας "data"
#αντίστοιχα έναν αριθμό, ο οποίος θα αντιπροσωπεύει την ετικέτα "with_mask" ή το "without_mask".
labels = []

#Εκτύπωση ενημερωτικού μηνύματος στην οθόνη.
print("[ΕΝΗΜΕΡΩΣΗ] Η φόρτωση των εικόνων ξεκίνησε...")

#Δημιουργία βρόγχου επανάληψης που την πρώτη φορά το "dataset_class" θα έχει την τιμή "with_mask"
#και τη δεύτερη/τελευταία φορά θα είναι "dataset_class" = "without_mask".
for dataset_class in dataset_classes:

	#Δημιουργία μεταβλητής αλφαριθμητικού τύπου η οποία περιέχει το αποτέλεσμα
	...
	for img_name in os.listdir(path):
		...
		#διαστάσεις που ορίζει το "target_size". Άρα με αυτήν την εντολή προσαρμόζονται όλες
		#οι εικόνες έτσι ώστε να έχουν την ίδια διάσταση αλλά με το ίδιο aspect ratio (ανάλυση)  που είχαν.
		image = load_img(img_path, target_size=(IMAGE_SIZE, IMAGE_SIZE))

		#Μετατροπή της εικόνας που βρίσκεται στο "image" σε μορφή διανύσματος για να μπορεί
		#να επεξεργαστεί αργότερα πιο εύκολα. Ένα κομμάτι του διανύσματος αυτού θα έχει για
		...
		#Επειδή για το CNN μοντέλο μας θα χρησιμοποιήσουμε την αρχιτεκτονική του μοντέλου
		#mobilenet_v2, χρειάζεται να χρησιμοποιήσουμε την εντολή "preprocess_input" πάνω
		#στο διάνυσμα της εικόνας μας. Η εντολή αυτή κάνει κάποιες μετατροπές και προσαρμογές
		#στο διάνυσμα έτσι ώστε αυτό να είναι συμβατό κατά την εκπαίδευση του μοντέλου μας.
		#Λαμβάνοντας υπόψιν το παράδειγμα απεικόνισης ενός μέρους του διανύσματος απο την προηγούμενη
		...
		#Προσθήκη με τη σειρά, όλων των διανυσμάτων των εικόνων στη λίστα "data" ώστε να είναι
		#αποθηκευμένες με αυτή τη μορφή σε ένα μέρος που θα μας διευκολύνει στην μετέπειτα
		#επεξεργασία τους.
		data.append(image)

		#Για κάθε μία απο τις εικόνες αποθηκεύεται αντίστοιχα και μια ετικέτα με την κατάσταση της
		#εικόνας. Δηλαδή, είτε "with mask" είτε "without_mask". Όλες οι ετικέτες αποθηκεύονται σε μία λίστα,
...
#από λίστα μετατρέπονται και αυτές σε διάνυσμα και ειδικότερα τα δεδομένα του που ήταν
...
#Αυτό γίνεται για να μπορούμε να χρησιμοποιήσουμε τις μεθόδους της προαναφερόμενης
#κλάσης πιο εύκολα.
lb = LabelBinarizer()
...
#με τον αριθμό 1. Επίσης το "labels" απο λίστα μετατρέπεται σε διάνυσμα (class numpy.ndarray int32) με δεδομένα της μορφής
#[0] ή [1].
labels = lb.fit_transform(labels)

#Η μέθοδος "to_categorial" χρησιμοποιεί την κωδικοποίηση One-hot encoding, η οποία μετατρέπει μια λίστα/διάνυσμα που περιέχει
#κατηγορίες όπως το "labels" σε μορφή τέτοια που μπορεί να χρησιμοποιηθεί εύκολα από αλγόριθμους μηχανικής
#μάθησης. Η βασική ιδέα της κωδικοποίησης αυτής είναι η δημιουργία νέων μεταβλητών που
...
#Μετατροπή, με τη βοήθεια της βιβλιοθήκης numpy (np), της λίστας "data" που περιέχει όλα τα διάνυσμα των εικόνων,
#σε ένα διάνυσμα και συγκεκριμένα τύπου float32.
data = np.array(data, dtype="float32")

#Το "labels" επειδή μετατράπηκε προηγουμένως σε διάνυσμα τύπου float32 δεν χρειάζεται θεωρητικά να εκτελεστεί η παρακάτω
...
#"labels" σε τέσσερα διανύσματα δύο κατηγοριών. Ουσιαστικά τα δεδομένα τους θα χωριστούν σύμφωνα με το ποσοστό που ορίζει η ιδιότητα
#"test_size" όπου στην προκειμένη περίπτωση είναι 0.2 ή αλλιώς 20%. Άρα το 20% των δεδομένων θα αποθηκευτεί στα διανύσματα
#"test_images" και "test_labels" που αφορούν τη κατηγορία testing (επικύρωσης) και το 80% θα αποθηκευτεί στα "train_images" και "train_labels" της κατηγορίας
...
#"stratify" δείxνει στο διάνυσμα των "labels" έτσι ώστε ο διαχωρισμός των δεδομένων στα "train_images", "test_images", "train_labels", "test_labels" να
...
#τότε το πρόγραμμα μπορεί να αποθήκευε τυχαία στα training διανύσματα μόνο τα δεδομένα που αντιστοιχούν σε labels=[0. 1] και έτσι
...
#αριθμό, ο οποίος συνήθως είναι το 42. Αυτός ο αριθμός ορίζει την τυχαιότητα που θα χωριστούν τα δεδομένα στα διανύσματα "train_images",
...
#Δημιουργία του αντικειμένου "aug_gen" από τη κλάση "ImageDataGenerator" της ενότητας t"ensorflow.keras.preprocessing.image".
...
#αναζητήσει κάποιος χειροκίνητα νέες εικόνες στο διαδίκτυο για να εμπλουτίσει το σύνολο δεδομένων. Η κλάση αυτή λαμβάνει
...
# model) στο παρελθόν. Η μεταβλητή "baseModel" μετατρέπεται σε  συνελικτικό νευρωνικό δίκτυο άρα και σε αντικείμενο (object) της ενότητας
...
#βάρη που προέκυψαν από την εκπαίδευση του MobileNetV2 πάνω στο σύνολο δεδομένων του "Imagenet" στο παρελθόν. To "Imagenet" είναι ένα
#τεράστιο σύνολο δεδομένων που περιέχει εκκατομύρια ετικέτες εικόνων διαφορετικών κατηγοριών. Εκπαιδεύοντας το μοντέλο μας με αυτά τα
...
#2)include_top: Με τη boolean τιμή False αφαιρούνται από την κορυφή του συνελικτικού νευρωνικού δικτύου τα κρυφά επίπεδα που περιέχουν τα πλήρως συνδεδεμένα επίπεδα (Fully Connected Layers)
#, τα οποία ήταν υπεύθυνα για την ταξινόμηση (classification) των τελικών αποτελεσμάτων. Αυτά τα συγκεκριμένα κρυφά επίπεδα
#καταργούνται διότι αργότερα θα προσθέσουμε τα δικά μας κρυφά επίπεδα που θα εκπαιδεύσουμε στο "headModel" και θα ταξινομούν τα δεδομένα
#στις κατηγορίες που θέλουμε εμείς για την συγκεκριμένη εργασία.
#3)input_tensor: Εδώ δηλώνονται οι διαστάσεις των εικόνων του συνόλου δεδομένων που θα εκπαιδευτεί το μοντέλο καθώς και το είδος των
...
#Δημιουργία του δεύτερου μέρους του μοντέλου που θα εκπαιδετεί. Αυτό ονομάζεται "headModel", το οποίο αποτελείται από τα
#κρυφά επίπεδα που προαναφέρθηκαν και είναι υπεύθυνο για την ταξινόμηση των δεδομένων και την σωστή επιλογή των τελικών αποτελεσμάτων
...
#που θα δεχτεί το "headModel" ονομάζεται χάρτης χαρακτηριστικών (feature map) και έχει τριδιάστατη μορφή 7x7x1280. Ο χάρτης χαρακτηριστικών περιέχει πληροφορίες
#για τα χαρακτηριστικά μιας εικόνας. Επίσης τα επίπεδα που θα χρησιμοποιηθούν βρίσκονται στην ενότητα "tensorflow.keras.layers".
headModel = baseModel.output

#Ενσωμάτωση του "AveragePooling2D" επιπέδου στο "headModel". Αυτό το επίπεδο μετατρέπει την μορφή του χάρτη χαρακτηριστικών από 7x7x1280
#σε 1x1x1280 εάν το pool_size επιλεχθεί (7,7). Αυτός ο μετασχηματισμός δέχεται κάθε κανάλι (channel) του 7x7x1280 με τη σειρά
#και από 49(7x7) pixels τα αλλάζει σε 1(1x1). Συγκεκριμένα υπολογίζεται ο μέσος όρος των τιμών που περιέχουν τα 49 pixels
#και αυτός αποθηκεύεται στη νέα μορφή του χάρτη χαρακτηριστικών. Κάποια απο τα πλεονεκτήματα αυτής της διαδικασίας είναι η μείωση των
#χωρικών διαστάσεων (spatial dimensions), η μείωση του θορύβου και η βελτίωση της υπολογιστικής απόδοσης.
headModel = AveragePooling2D(pool_size=(7, 7))(headModel)

#Ενσωμάτωση του "Flatten" στο "headModel". Το επίπεδο αυτό δέχεται το αποτέλεσμα του "AveragePooling2D" καλώντας δίπλα από την εντολή
#"Flatten(name="flatten")" το "(headModel)". Συγκεκριμένα δέχεται το χάρτη χαρακτηριστικών της μορφής 1x1x1280 το οποίο έχει 1280 κανάλια
#και το μετατρέπει σε ένα διάνυσμα (τύπου vector) μίας μόνο διάστασης, δηλαδή 1x1280. Αυτή η διαδικασία βοηθάει κυρίως στη συμβατότητα,
#αφού από το επόμενο βήμα που θα ακολουθήσουν τα πλήρψς συνδεδεμένα επίπεδα, αυτά θα δεχτούν στην είσοδό τους των χάρτη χαρακτηριστικών που θα πρέπει να έχει
#τη μορφή μονοδιάστατου διανύσματος. Ειδικότερα το επίπεδο "Flatten" παίζει κρίσιμο ρόλο στη μετάβαση από τα επίπεδα του συνελικτικού νευρωνικού δικτύου στα
#κρυφά επίπεδα του νευρωνικού δικτύου. Ακόμα, κάθε στοιχείο του διανύσματος πλέον θεωρείται ως ένας νευρώνας, άρα τα πλήρψςς συνδεδεμένα επίπεδα
#θα δεχτούν 1280 τιμές ως εισόδους σε κάθε νευρώνα που περιέχει το επόμενο επίπεδο.
headModel = Flatten(name="flatten")(headModel)

#Δημιουργία ενός πλήρως συνδεδεμένου επιπέδου. Το "Dense" δέχεται ως είσοδο τις 1280 τιμές του διανύσματος που δημιούργησε το "Flatten" αναγνωρίζοντας
#αυτές ως αρχικούς νευρώνες και δημιουργεί ένα πλήρψς συνδεδεμένο επίπεδο που αποτελείται απο 128 νέους νευρώνες. Κάθε νέος νευρώνας από τους
...
#Εφαρμογή της μεθόδου "Dropout" στις τιμές που παρήγαγαν οι νευρώνες του προηγούμενου επιπέδου. Το dropout είναι μία τεχνική τακτοποίησης
#(regularization technique) που απορρίπτει τυχαία το 50% των εξόδων των νευρώνων από το προηγούμενο layer κατά τη διάρκεια
#της εκπαίδευσης του τελικού μοντέλου. Αυτό βοηθά στην αποφυγή της υπερπροσαρμογής (overfitting) που είναι η κατάσταση στην οποία
...
#Δημιουργία του δεύτερου και τελευταίου πλήρως συνδεδεμένου επιπέδου. Το "Dense" αυτή τη φορά δέχεται στην είσοδό του τις εξόδους του προηγούμενου
#πλήρως συνδεδεμένου επιπέδου με τους 128 νευρώνες, των οποίων οι μισές τιμές θα είναι πλέον μηδενικές λόγω του "Dropout". Το "Dense" εδώ δημιουργεί
...
#εικόνα που επεξεργάστηκε ανήκει στην κατηγορία "with mask" ή "without mask". Όπως και στο προηγούμενο πλήρως συνδεδεμένο επίπεδο, έτσι και εδώ
...
#Αναλόγως την πιθανότητα και το ποσοστό επιτυχίας του, το νευρωνικό δίκτυο ενημερώνει τα βάρη και τις πολώσεις του σε κάθε
...
#του χάρτη χαρακτηριστικών από τις εικόνες του training dataset και του "headModel" που περιέχει τα κρυφά επίπεδα, τα οποία είναι υπεύθυνα
#για την ταξινόμηση του χάρτη χαρακτηριστικών στις κατηγορίες "mask" ή "without mask". Μετά την σύνδεση του "headModel" στην κορυφή του
#"baseModel" το ολοκληρωμένο νευρωνικό δίκτυο που θα εκπαιδεύσουμε θα έχει την ονομασία "model". Η ένωση επιτυγχάνεται με την
#εντολή "Model()" που βρίσκεται στην ενότητα "tensorflow.keras.models".
model = Model(inputs=baseModel.input, outputs=headModel)

#Απενεργοποίηση της δυνατότητας εκπαίδευσης όλων των επιπέδων του "baseModel" αφού είναι ήδη εκπαιδευμένο και δεν θέλουμε να
#υποστεί κάποια αλλαγή στα βάρη ή γενικότερα στις παραμέτρους του. Αντίθετα, το "headModel" είναι αυτό που θα εκπαιδευτεί
#εξ' ολοκλήρου και από την αρχή γιατί εκείνο αφορά την ταξινόμηση στις δύο κλάσεις που επιθυμούμε για αυτή τη διπλωματική εργασία.
...
#τέλος κάθε επανάληψης υπολογίζεται η συνάρτηση απωλειών (loss function) συγκρίνοντας το αποτέλεσμα που υπολογίστηκε σε
#σχέση με το πραγματικό αποτέλεσμα. Έπειτα υπολογίζονται οι κλίσεις (gradients) για κάθε παράμετρο της συνάρτησης απωλειών με μία
#μέθοδο που ονομάζεται οπισθοδιάδοση (backprobagation). Αυτή η μέθοδος περιλαμβάνει τον υπολογισμό των μερικών παραγώγων όλων των παραμέτρων
#(weights και bias) πάνω στη συνάρτηση απωλειών χρησιμοποιώντας τον κανόνα της αλυσίδας (chain rule). Αφού υπολογιστεί η συνάρτηση απωλειών
# και οι κλίσεις, τα δέχεται ο Adam σαν δεδομένα και με τη σερά του υπολογίζει τις νέες τιμές που θα πάρουν οι
#παράμετροι κατά την επόμενη επανάληψη της εκπαίδευσης. Ο Adam συνδυάζει ιδέες τόσο από μεθόδους που βασίζονται στην ορμή
#(momentum) όσο και από μεθόδους προσαρμοστικού ρυθμού εκπαίδευσης (initial learning rate) για να ενημερώσει τις παραμέτρους ενός μοντέλου.
...
#ενημέρωσή του, όπου δίνονται πληροφορίες για το είδος της συνάρτησης απωλειών (binary_crossentropy) που επιθυμούμε να χρησιμοποιήσει,
...
#Εκπαίδευση του head μέρους του μοντέλου, δηλαδή του "headModel" που περιέχει τα κρυφά επίπεδα, με την μέθοδο ".fit()". Κάποιες
#πληροφορίες σχετικά με την εκπαίδευση θα αποθηκευτούν στην μεταβλητή "HISTORY" όπου θα περιέχει κατηγορίες δεδομένων που ορίστηκαν
#κατά τη μεταγλώτισση του μοντέλου με το "metrics=["accuracy"]". Κατά την κλήση της μεθόδου ".fit()" δίνονται τιμές σε κάποιες ιδιότητες
...
#μοντέλο (training set) μαζί με το μέγευος υποσυνόλου δεδομένων (BS) που έχουμε ορίσει στην αρχή του κώδικα και είναι ο αριθμός που θα χωρίσει
...
#(data augmentation) εφαρμόζοντάς τους αυτές τις τροποποιήσης. Έτσι θα δεχτεί το ".fit()" τα νέα "train_images" και "train_labels" set τα οποία
...
#εικόνων του training set με το "BS". Αυτό διασφαλίζει ότι ολόκληρο το training set θα χρησιμοποιείται σε κάθε επανάληψη.
...
#όλων των εικόνων του testing set με το "BS". Και εδώ γίνεται διασφάλιση του ότι θα χρησιμοποιηθούν όλα τα δεδομένα
...
#την ενότητα "tensorflow.keras.models".
...
#".tflite". Τα μοντέλα ".tflite" φτιάχνονται για να λειρουργούν πιο γρήγορα και πιο αποτελεσματικά στα λειτουργικά συστήματα των συσκευών άκρης (edge devices)
#, όπως για παράδειγμα ένα Raspberry Pi 4. Γι' αυτό ένα μοντέλο ".tflite" εάν χρησιμοποιηθεί π.χ. σε υπολογιστή με Windows 10, θα αργεί περισσότερο τον κώδικα
...
#μορφή όλα αυτά τα αρχεία τοποθετούνται σε έναν φάκελο. Έτσι η μορφή ".h5" είναι πιο βολική στην περίπτωση της συγκεκριμένης διπλωματικής εργασίας.
...
#σε μορφή ".tflite". Η φόρτωση γίνεται καλώντας την μέθοδο "load_model" της ενότητας "tensorflow.keras.models" και δίνοντάς
...
#Το "converter.optimizations" ενεργοποιεί τη βελτιστοποίηση του νέου μοντέλου, που είναι μία διαδικασία πολύ σημαντική και
...
#του testing set χωρίζοντας αυτες σε ίσες ομάδες (Batches) και αποθηκεύει στη μεταβλητή "predictions" τις προβλέψεις ως πιθανότητες.
predictions = model.predict(test_images, batch_size=BS)

#Δημιουργία διανύσματος, που θα περιέχει τις ετικέτες του testing set, σε μορφή τέτοια έτσι ώστε να την επεξεργαστεί παρακάτω
#η μέθοδος "roc_curve" χωρίς προβλήματα συμβατότητας.
test_labels_binary = np.argmax(test_labels, axis=1)

#Δημιουργία διανύσματος, που θα περιέχει τις προβλέψεις που έγιναν για το μοντέλο πάνω στις εικόνες του testing set, σε μορφή
...
#"predictions_binary" που δημιουργήθηκαν παραπάνω και περιέχουν τις εικόνες του testing set καθώς και τις ετικέτες τους σε δυαδική μορφή
#. Τα FPR, TPR και τα thresholds τους θα δημιουργήσουν παρακάτω στον κώδικα το διάγραμμα της καμπύλης ROC (Receiver Operating
#Characteristic).
fpr, tpr, thresholds = roc_curve(test_labels_binary, predictions_binary)

#Υπολογισμός του AUC (Area Under the ROC Curve), που είναι η περιοχή κάτω απο την ROC καμπύλη, από τα "fpr" και "tpr" χρησιμοποιώντας την μέθοδο "auc()" της
#βιβλιοθήκης scikit-learn. Ο αριθμός ή βαθμολογία AUC δείχνει την απόδοση του μοντέλου.
...
#εικόνα της πρώτης ομάδας (batch) του "test_images" έγινε η πρόβλεψη με αποτέλεσμα 0.3 (30%) πιθανότητα για το "with_mask" και 0,7(70%) πιθανότητα
...
#ανάκληση (recall), η βαθμολογία F1 (F1-score), η ακρίβεια (precision), η ορθότητα (accuracy),ο μεσος όρος (macro average) αυτών σε
#όλες τις κλάσεις καθώς και ο σταθμισμένος μέσος όρος (weighted average) αυτών.
#Για τη διαδικασία παραγωγής όλων των παραπάνω πληροφοριών είναι υπεύθυνη η εντολή "classification_report()"
#της ενότητας "sklearn.metrics". Αυτή η εντολή δέχεται αρχικά ως δεδομένα τις πραγματικές τιμές των ετικετών που αντιστοιχούν
#στο "test_images" σύνολο δεδομένων. Τα δεδομένα αυτά βρίσκονται θεωρητικά στο διάνυσμα "test_labels" αλλά επειδή αυτά τα δεδομένα είναι κωδικοποιηνμένα με τη μορφή
...
#η ".argmax()" θα ελέγξει κάθε γραμμή του διανύσματος "test_labels" αφού το "axis" έχει ορισθεί ως 1 και θα επιστρέψει τον δείκτη με τη μεγαλύτερη
...
#τιμές του "test_labels". Τέλος ενεργοποιείται η ιδιότητα "target_names" στην οποία δίνεται η τιμή lb.classes_ που επιστρέφει το διάνυσμα
#με τις αλφαριθμητικές ονομασίες των δύο ομάδων από ετικέτες (with_mask και without_mask). Αυτές δίνονται στο "classification_report" έτσι
#ώστε αυτό να προβάλλει τις πληροφορίες με περισσότερη αναγνωσιμότητα. Παρακάτω αναλύονται οι μετρικές αξιολόγησης που θα εκτυπωθούν:
...
#3) βαθμολογία F1 (F1-score): Μετρά την ακρίβεια ενός μοντέλου συνδυάζοντας τις τιμές της ανάκλησης και της ακρίβειας που
#έχουν ήδη υπολογιστεί. Ειδικότερα, παρέχει μία ισορροπημένη μέτρηση της απόδοσης του μοντέλου, ειδικά όταν υπάρχει
#ανισορροπία μεταξύ του αριθμού των δειγμάτων σε διαφορετικές κλάσεις.
#4) Ορθότητα (accuracy): Υπολογίζεται ως η αναλογία μεταξύ του αριθμού των σωστών προβλέψεων (θετικών και αρνητικών σωστών
...
#5) Μακρύς μέσος όρος (macro average): Βρίσκει τον μέσο όρο κάθε παραπάνω μετρικού μεταξύ όλων των κλάσεων.
#6) Σταθμισμένος μέσος όρος (weighted average): Και αυτός υπολογίζει τον μέσο όρο τον μετρικών αλλά δίνει βαρύτητα στην
#ποσότητα των δεδομένων κάθε κλάσης έτσι ώστε να γίνει μια πιο δίκαιη μέτρηση.
report = classification_report(test_labels.argmax(axis=1), predictions, target_names=lb.classes_)
print(report)

#Δημιουργία ενός νέου αρχείου κειμένου με όνομα "classification_report.txt" και άνοιγμά του για εγγραφή πληροφοριών μέσα
#σε αυτό.
with open(f'{folder_of_model}/classification_report.txt', 'w') as file:
	#Αποθήκευση των πληροφοριών που παρήχθησαν απο το "classification_report" σε αρχείο .txt .
	file.write(report)
...
#Υπολογισμός των τελικών τιμών απωλειών και ορθοτήτων για την γραφική απεικόνισή τους πάνω στα διαγράμματα που θα φτιαχτούν.
#Συνολικά θα δημιουργηθούν τέσσερις μεταβλητές αφου υπάρχουν δύο τελικές τιμές για τις απώλειες, μια για ττα δεδομένα εκπαίδευσης
#(training set) και μία για τα δεδομένα ελέγχου/επικύρωσης (testing/validating set), καθώς και δύο για την ορθότητα, μια για τα δεδομένα
#εκπαίδευσης και μια για αυτά τηε επικύρωσης. Οι τιμές αυτές
...
#όρισμα υπάρχει το [-1] που δηλώνει ποια από όλες τις τιμές της κάυε μέτρησης θέλουμε να υπολογίσουμε. Επειδή η κάθε μέτρηση
#έχει τη μορφή διανύσματος, για ένα διάνυσμα το [-1] σημαίνει η τελευταία τιμή του στη γλώσσα προγραμματισμού Python.
...
#Ορισμός μεγέθους του πλαισίου (figure) που θα περιέχει το διάγραμμα και συγκεκριμένα με τη μέθοδο ".figure()" και ορίσματα "8,6". Αυτά τα ορίσματα
#μετριούνται σε ίντσες και επιλέχθηκαν μετά απο διάφορες δοκιμές διότι εμφάνιζαν καλύτερα τα δεδομένα και τους άξονες.
plt.figure(figsize=(8, 6))

#Εισαγωγή των δεδομένων της μέτρησης "loss" (που αφορά την εκπαίδευση) στον άξονα y του διαγράμματος και στις τιμές του άξονα x
#τοποθετούνται οι αριθμοί 1 έως "EPOCHS",δηλαδή κάθε εποχή/επανάληψη εκπαίδευσης. Έτσι για κάθε εποχή απεικονίζεται η αντίστοιχη
...
#τα δεδομένα της μέτρησης "val_loss" (αφορούν τα δεδομένα επικύρωσης) και τους δίνουμε την ονομασία "validation loss".
...
#"loss" (εκπαίδευσης και επικύρωσης) καθώς και το χρώμα που αντιπροσωπεύει κάθε γραμμή. Το υπόμνημα με την ιδιότητα "loc"
...
#Εδώ γίνεται επισήμανση πάνω στο διάγραμμα σχετικά με την τελευταία τιμή των απωλειών επικύρωσης (final_val_loss) ακριβώς με τον
#ίδιο τρόπο όπως και για την τελική τιμή των απωλειών εκπαίδευσης.
...
#Αποθήκευση του διαγράμματος με την μέθοδο ".savefig()" στον επιθυμητό φάκελο σε μορφή εικόνας τύπου ".png", με ανάλυση 300 dpi
...
#Διαδικασία δημιουργίας του 2ου διαγράμματος που θα περιέχει τις ορθότητες εκπαίδευσηςκαι τις ορθότητες επικύρωσης.
#Για τη δημιουργία του 2ου διαγράμματος ακολουθείται ακριβώς η ίδια διαδικασία με το πρώτο διάγραμμα αλλάζοντας μόνο τα
#δεδομένα του διαγράμματος και το όνομα του αρχείου που θα αποθηκευτεί.
plt.style.use("bmh")
plt.figure(figsize=(8, 6))
...
#Διαγράμματα με σκούρο παρασκήνιο. Τα παρακάτω δύο διαγράμματα (3ο και 4ο) είναι ακριβώς ίδια με το 1ο και 2ο, με τη μόνη
...
#Διαδικασία δημιουργίας του 3ου διαγράμματος που θα περιέχει τα τις απώλειες εκπαίδευσης και απώλειες επικύρωσης.
...
#Διαδικασία δημιουργίας του 4oυ διαγράμματος που θα περιέχει τις ορθότητες εκπαίδευσης και τις ορθότητες επικύρωσης.
