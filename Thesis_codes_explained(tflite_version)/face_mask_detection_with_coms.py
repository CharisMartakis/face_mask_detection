...
#κάμερα του Raspberry Pi 4. Η επιλογή της κάμερας γίνετσαι μέσα στις παρενθέσεις του "VideoStream()" όπου δέχεται το όρισμα
...
#κάμερα ξελινά να λαμβάνει στιγμυότυπα (Frames).
...
#Δημιουργία ατέρμονου βρόγχου επανάληψης "while" αφού κατά τον έλεγχό του δέχεται πάντα την απάντηση της συνθήκης ως "True".
#Μέσα σε αυτόν τον βρόγχο πραγματοποιήται αρχικά σε κάθε επανάληψη του η λήψη ενός στιγμιοτύπου "frame" απο την κάμερα.
...
#γύρω απο το ανθρώπινο πρόσωπο με χρώμα κόκκινο εάν δεν φοράει μάσκα ή πράσινο εάν φοράει. Επιπλέον πάνω ακριβώς απο αυτό
...
#δεξιά σημείο του παραθύρου. Εφόσον ένα απο τα δύο πατήθηκε, τότε το παράθυρο κλείνει και το πρόγραμμα τερματίζεται. Σε
#περίπτωση που δεν κλέισει το παράθυρο τότε ο κώδικας μέσα στον βρόγχο επανάληψης συνεχίζει να εκτελείται λαμβάνοντας το
#επόμενο στιγμιότυπο.
while True:

	#Έλεγχος με τη συνθήκη "if" εάν η μεταβλητή "fl" είναι ίση με το "1". Αυτός ο έλεγχος γίνεται ώστε της πρώτη φορά που
	...
	if fl == 1:
		...
		#θα ελέγχεται κάθε φορά. Εδώ γίνεται έλεγχος εάν το παράθυρο, που εμφανίζονται τα "frames" ως βίντεο, έκλεισε απο
		...
		#επιστρέφει μια τιμή κάτω του "1" αν  το παράθυρο έχει κλείσει ή μεγαλύτερο ή και ίσο με το ένα άν παραμένει ανοικτό.
		...
		if cv2.getWindowProperty("Mask Detection", cv2.WND_PROP_VISIBLE) < 1:
			...
			#επανάληψης "while" σταματάει να εκτελείται σε αυτό το σημείο και συνεχίζει να εκτελείται απο το σημείο που η
			#"while" τελειώνει. Η ένοια «τελειώνει» εννοεί ότι ο κώδικας συνεχίζει να εκτελείται μετά την τελευταία εμφωλευμένη
			#εντολή που ανήκει στη "while".
			break
	#Η "else" συνθήκη θα εκτελεστεί μόνο μια φορά και συγκεκριμένα στην πρώτη επανάληψη του βρόγχου "while" επειδή κατά
	#τον έλεγχο του "fl" αυτό θα είναι διάφορο του "1" και συγκεκριμένα "0".
	else:
		#Η εντολή που θα εκτελεστεί μια φορά μόνο, είναι η αλλαγή της τιμής της μεταβλητής "fl" απο "0" σε "1" άρα η ενεργοποίηση
		#απο την επόμενη επανάληψη του βρόγχου "while" του ελέγχου για το κλείσιμο ή μη του παραθύρου που προβάλλει τα "frames".
		fl = 1

	#Με τηβ εντολή ".read()" πάνω στο αντικείμενο "vs" λαμβάνεται το στιγμιότυπο της κάμερας εκείνης της δεδομένης στιγμής
	...
	#γύρω απο τα ανθρώπινα πρόσωπα καθώς και τις προβλέψεις ανίχνευσης ή μη μάσκας.
	(locs, preds) = detect_and_predict_mask(frame, faceNet, maskNet)

	#Δημιουργία βρόγχου επανάληψης "for" που θα ελέγχει κάθε περίπτωση εντοπισμού προσώπου μέσα στο "frame", άρα και
	#πρόβλεψη ανίχνευσης ή μη μάσκας σε αυτό, ξεχωριστά και θα εμφανίζει τις απαραίτητες πληροφορίες για αυτά. Ειδικότερα
	...
	#καταχωρούνται απο αυτά στα "box" και "pred" να είναι αντισοιχισμένες μεταξύ τους και να μην μπερδευτούν.
	for (box, pred) in zip(locs, preds):
		...
		#του κόκκινου χρώματος δηλαδή "(0, 0, 255)". To "color" περιέχει το χρώμα του παραλληλόγραμμου που θα σχεδιαστεί
		#γύρω απο το ανθρώπινο πρόσωπο.
		...
		#ποσοστού. Το "{}: {:.2f}%" είναι το string που θα εμφανίζεται στην οθόνη πάνω απο κάθε ανθρώπιο πρόσωπο όπου το
		...
		#"label" ενώ ως δεύτερο όρισμα την μεγαλύτερη πιθανότητα απο τις "mask" και "withoutMask" πολλαπλασιασμένη
		#επί "100".
		label = "{}: {:.2f}%".format(label, max(mask, withoutMask) * 100)

		#Με την εντολή ".putText()" της βιβλιοθήκης OpenCV εμφανίζεται πάνω στο "frame" το κείμενο που περιέχει το "label".
		#Επειδή όμως το κείμενο δεν μπορεί να εμφανιστεί σε οποιαδήποτε σημείο με οποιαδήποτε χρώμα κλπ, αυτά όλα ρυθμίζονται
		#απο τα ορίσματα που δίνονται στο ".putText()". Το πρώτο όρισμα είναι το "frame" στο οποίο θα εμφανιστεί το κείμενο.
		...
		#πάνω αριστερά γωνία του ορθογωνίου παραλληλόγραμμου. Το τέταρτο είναι η επιλογή της γραμματοσειράς του κειμένου
		...
		#η εμφάνιση του ορθογώνιου παραλληλογράμου περιγράμματος γύρω απο ένα ανθρώπινο πρόσωπο. Εδώ το όρισμα κειμένου
		#έχει αφαιρεθεί αφου δεν υπάρχει όπως και η γραμματοσειρά μαζί με το μέγεθός της. Παρόλα αυτά έχει επιλεχθεί το ίδιο
...
#Η εντολή ".destroyAllWindows()" της OpenCV κλείνει ότι παράθυρο υπάρχει ανοικτό στην μνήμη του υπολογιστή και έπειτα το
