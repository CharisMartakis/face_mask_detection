...
#Το "tensorflow" είναι ένα πλαίσιο ανοικτού κώδικα (open-source code) όπου η βιβλιοθήκη του περιέχει εντολές με σκοπό τη
...
#Εισαγωγή της συνάρτησης "preprocess_input" από την "tensorflow.keras.applications.mobilenet_v2" ενότητα. Η συνάρτηση αυτή
...
#Εισαγωγή της συνάρτησης "img_to_array" από την "tensorflow.keras.preprocessing.image" ενότητα. Η συνάρτηση αυτή, μετατρέπει
...
#Εισαγωγή της κλάσης "VideoStream" από την "imutils.video" ενότητα. Η κλάση αυτή παρέχει μία απλοποιημένη διεπαφή για πρόσβαση
...
#To "imutils" είναι μια βιβλιοθήκη που περιέχει συναρτήσεις για την διευκόλυνση της χρήσης της βιβλιοθήκης OpenCV (Open Source
...
#Το "cv2" αναφέρεται στην βιβλιοθήκη ανοικτού κώδικα "OpenCV" η οποία χρησιμοποιείται ευρέως για θέματα
...
#Δημιουργία της κενής λίστας "locs" στην οποία θα αποθηκευτούν οι συντεταγμένες των παραλληλογράμμων που θα τοποθετηθούν γύρω
#από τα πρόσωπα που μπορεί να βρέθηκαν.
	locs = []

#Δημιουργία της κενής λίστας "preds" στην οποία θα αποθηκευτούν οι προβλέψεις για το αν εντοπίστηκε μάσκα ή όχι στα πρόσωπα που
#μπορεί να βρέθηκαν.
	preds = []
...
#μέσω της μεθόδου ".astype("int")" και έπειτα αποθηκεύονται σε μια πλειάδα και ειδικότερα η κάθε μία σε μία
...
#Μετατροπή της εικόνας "face" σε μορφή διανύσματος με τη χρήση της συνάρτησης "img_to_array()" από την
#"tensorflow.keras.preprocessing.image" ενότητα. Αυτή η μετατροπή είναι απαράιτητη για την προετοιμασία
#του "face" για περαιτέρω επεξεργασία από το μοντέλο ανίχνευσης μάσκας.
			face = img_to_array(face)

#Με τη συνάρτηση "preprocess_input()" από την "tensorflow.keras.applications.mobilenet_v2" ενότητα γίνονται
...
#Αποθήκευση της τοποθεσίας του αρχείου "deploy.prototxt" ως "r" (raw) αλφαριθμητικό στη μεταβλητή "prototxtPath". Το "deploy.prototxt"
...
#έχει σχεδιαστεί με βάση των αλγόριθμο Single Shot MultiBox Detector (SSD) και περιέχει στοιχεία από την αρχιτεκτονική του "MobileNetV1".
...
#ως "r" (raw) αλφαριθμητικό στη μεταβλητή "weightsPath". Αυτό το αρχείο περιέχει πληροφορίες σχετικά με τα βάρη του μοντέλου και τις
...
#Αποθήκευση της τοποθεσίας του αρχείου "mask_detection_model_optim.tflite" ως "r" (raw) αλφαριθμητικό στη μεταβλητή "mask_string". Το
...
#στο αντικείμενο "input_details". Πιο συγκεκριμένα, θα αποθηκευτεί μία λίστα απο λεξικά (dictionaries) όπου κάθε λεξικό (dictionary) αντιστοιχεί
...
#λεξικά όπου κάθε λεξικό αντιστοιχεί σε έναν τανυστή εξόδου.
...
#Εάν λοιπόν είναι μικρότερο του "1" τότε ο κώδικας μέσα σε αυτό το "if" παραβλέπεται και η εκτέλεση του προγράμματος
...
#Με την συνάρτηση ".resize()" της ενότητας "imutils" αλλάζουν οι διαστάσεις της εικόνας "frame" σε τέτοιες ώστε να είναι
...
#επιστρέφει σε μορφή πλειάδας τις λίστες "locs" και "preds" που περιέχουν αντίστοιχα τις συντεταγμένες των νοητών παραλληλογράμων
